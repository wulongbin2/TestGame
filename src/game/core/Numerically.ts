module gameCore {
	...
	export function calculHeroZDL(vo:HeroMO):number{
		return (vo.roleLevel*(vo.awakenLevel+1)+vo.awakenLevel*gamesystem.AwakenLevel)*100*vo.roleVo.potential;
	}
	/**计算角色下一级战力 */
	export function calculHeroNextLevelZDL(vo:HeroMO):number{
		var roleLevel:number = vo.roleLevel+1;
		return (roleLevel*(vo.awakenLevel+1)+vo.awakenLevel*gamesystem.AwakenLevel)*100*vo.roleVo.potential;
	}
	/**计算角色下一集战力增加 */
